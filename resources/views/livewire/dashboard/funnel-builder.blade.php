<div class="grid gap-4 lg:grid-cols-12" wire:key="builder-{{ $funnel->id }}">
    ...
    <section class="lg:col-span-3 rounded-xl border border-gray-200 bg-white p-4">
        <div class="flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-900">{{ __('funnel.builder.steps') }}</h2>
            <button type="button" class="btn btn-xs btn-primary" wire:click="addStep">
                {{ __('funnel.builder.add_step') }}
            </button>
        </div>
        ...
        <ul class="mt-3 space-y-1"
            ...
            @forelse ($steps as $step)
                <li wire:key="step-{{ $step->id }}" data-id="{{ $step->id }}"
                    ...
                    <span class="cursor-grab select-none text-gray-400" data-drag-handle title="{{ __('funnel.builder.drag') }}">⠿</span>
                    ...
                    <button type="button" class="text-xs text-red-600 hover:underline"
                            ...
                            wire:confirm="{{ __('funnel.builder.confirm_delete_step') }}">
                        {{ __('funnel.builder.delete') }}
                    </button>
                </li>
            @empty
                <li class="px-2 py-4 text-sm text-gray-500">{{ __('funnel.builder.no_steps') }}</li>
            @endforelse
        </ul>
    </section>
    ...
    <section class="lg:col-span-5 rounded-xl border border-gray-200 bg-white p-4">
        @if ($selectedStepId === null)
            <p class="text-sm text-gray-500">{{ __('funnel.builder.select_step') }}</p>
        @else
            <div>
                <label for="step-title" class="block text-xs font-medium text-gray-700">
                    {{ __('funnel.builder.step_title') }}
                </label>
            ...
            <div class="mt-4 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-gray-900">{{ __('funnel.builder.questions') }}</h2>
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" class="btn btn-xs btn-primary"
                        ...
                        {{ __('funnel.builder.add_question') }}
                    </button>
                </div>
            </div>
                ...
                @forelse ($questions as $item)
                    <li wire:key="question-{{ $item->id }}" data-id="{{ $item->id }}"
                        ...
                        <button type="button" class="text-xs text-red-600 hover:underline"
                                ...
                                wire:confirm="{{ __('funnel.builder.confirm_delete_question') }}">
                            {{ __('funnel.builder.delete') }}
                        </button>
                    </li>
                @empty
                    <li class="px-2 py-4 text-sm text-gray-500">{{ __('funnel.builder.no_questions') }}</li>
                @endforelse
            </ul>
        @endif
    </section>
    ...
    <section class="lg:col-span-4 rounded-xl border border-gray-200 bg-white p-4">
        @if ($question === null)
            <p class="text-sm text-gray-500">{{ __('funnel.builder.select_question') }}</p>
        @else
            <h2 class="text-sm font-semibold text-gray-900">{{ __('funnel.builder.properties') }}</h2>
            ...
            <div class="mt-3 space-y-3">
                <div>
                    <label for="q-type" class="block text-xs font-medium text-gray-700">{{ __('funnel.builder.type') }}</label>
                ...
                <div>
                    <label for="q-label" class="block text-xs font-medium text-gray-700">{{ __('funnel.builder.label') }}</label>
                ...
                <div>
                    <label for="q-key" class="block text-xs font-medium text-gray-700">{{ __('funnel.builder.field_key') }}</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500">{{ __('funnel.builder.field_key_hint') }}</p>
                    @error('questionForm.field_key') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="q-help" class="block text-xs font-medium text-gray-700">{{ __('funnel.builder.help_text') }}</label>
                ...
                <label class="flex items-center gap-2 text-sm">
                    ...
                    {{ __('funnel.builder.required') }}
                </label>

                <p class="text-xs text-gray-500">
                    {{ __('funnel.builder.rendered_by', ['component' => $previewComponent]) }}
                </p>
            </div>

            <div class="mt-5">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-gray-900">{{ __('funnel.builder.options') }}</h3>
                    <button type="button" class="btn btn-xs" wire:click="addOption">
                        {{ __('funnel.builder.add_option') }}
                    </button>
                </div>
                    ...
                    @forelse ($options as $option)
                        <li wire:key="option-{{ $option->id }}" class="flex items-center gap-2 text-sm">
                            ...
                            <button type="button" class="text-xs text-red-600 hover:underline"
                                    ...
                                    wire:confirm="{{ __('funnel.builder.confirm_delete_option') }}">
                                {{ __('funnel.builder.delete') }}
                            </button>
                        </li>
                    @empty
                        <li class="text-sm text-gray-500">{{ __('funnel.builder.no_options') }}</li>
                    @endforelse
                </ul>
            </div>
        @endif
