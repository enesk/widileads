...
<div class="max-w-2xl mx-auto">
    @if ($registration !== null)
        <div class="card bg-base-100 shadow">
            <div class="card-body">
                <h1 class="card-title">{{ __('funnel.buyer.form.received_heading') }}</h1>

                <p>{{ __('funnel.buyer.form.received_text', ['company' => $registration->company_name]) }}</p>
                ...
                @if ($registration->status->isPending())
                    <p class="mt-4 text-sm opacity-70">{{ __('funnel.buyer.form.received_hint') }}</p>
                @endif
    ...
    @else
        <div class="card bg-base-100 shadow">
            <div class="card-body">
                <h1 class="card-title">{{ __('funnel.buyer.form.heading') }}</h1>
                <p class="text-sm opacity-70">{{ __('funnel.buyer.form.description') }}</p>

                <form wire:submit="submit" class="mt-6 space-y-4">
                    <div class="form-control">
                        <label class="label" for="company-name">
                            <span class="label-text">{{ __('funnel.buyer.form.company_name') }}</span>
                        </label>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="form-control">
                            <label class="label" for="contact-name">
                                <span class="label-text">{{ __('funnel.buyer.form.contact_name') }}</span>
                            </label>
                        ...
                        <div class="form-control">
                            <label class="label" for="contact-email">
                                <span class="label-text">{{ __('funnel.buyer.form.contact_email') }}</span>
                            </label>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="form-control">
                            <label class="label" for="contact-phone">
                                <span class="label-text">{{ __('funnel.buyer.form.contact_phone') }}</span>
                            </label>
                        ...
                        <div class="form-control">
                            <label class="label" for="vat-id">
                                <span class="label-text">{{ __('funnel.buyer.form.vat_id') }}</span>
                            </label>
                    ...
                    <div class="form-control">
                        <label class="label" for="broker-register-number">
                            <span class="label-text">{{ __('funnel.buyer.form.broker_register_number') }}</span>
                        </label>
                        <input id="broker-register-number" type="text" wire:model="brokerRegisterNumber"
                               class="input input-bordered w-full">
                        <span class="text-sm opacity-70 mt-1">
                            {{ __('funnel.buyer.form.broker_register_number_helper') }}
                        </span>
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-3">
                            ...
                            <span class="label-text">{{ __('funnel.buyer.form.av_accepted') }}</span>
                        </label>
                        <span class="text-sm opacity-70">{{ __('funnel.buyer.form.av_accepted_helper') }}</span>
                        @error('avAccepted')
                            <span class="text-error text-sm mt-1">{{ $message }}</span>
                        @enderror
                    </div>
