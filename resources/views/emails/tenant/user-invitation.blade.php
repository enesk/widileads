{{-- Einladung in einen Workspace. Aufbau wie die uebrigen Portalmails. --}}
@php
...
<x-layouts.email>
    <x-slot name="preview">
        {{ __('mail.invitation.heading', ['tenant' => $invitation->tenant->name]) }}
    </x-slot>
                ...
                <tr>
                    <td class="sm-px-6" style="padding: 32px 40px; font-size: 16px; color: #334155">
                        <p style="margin: 0; font-size: 16px; line-height: 24px; color: #334155">
                            {{ __('mail.invitation.intro', ['tenant' => $invitation->tenant->name]) }}
                        </p>
                        <table style="width: 100%; margin: 28px 0" cellpadding="0" cellspacing="0" role="none">
                            <tr>
                                <td align="center">
                                    <a href="{{ route('invitations') }}" style="display: inline-block; border-radius: 10px; background-color: {{ $tint }}; padding: 14px 32px; font-size: 16px; font-weight: 600; color: #ffffff; text-decoration: none">
