{{-- Fehlgeschlagene Abbuchung eines Abonnements. --}}
@php
...
<x-layouts.email>
    <x-slot name="preview">
        {{ __('mail.payment_failed.heading') }}
    </x-slot>
                ...
                <tr>
                    <td class="sm-px-6" style="padding: 32px 40px; font-size: 16px; color: #334155">
                        ...
                        <p style="margin: 0; font-size: 16px; line-height: 24px; color: #334155">
                            {{ __('mail.payment_failed.intro', ['plan' => $subscription->plan->name]) }}
                        </p>
                        <table style="width: 100%; margin: 28px 0" cellpadding="0" cellspacing="0" role="none">
                            <tr>
                                <td align="center">
                                    <a href="{{ route('filament.dashboard.resources.subscriptions.index', ['tenant' => $subscription->tenant]) }}" style="display: inline-block; border-radius: 10px; background-color: {{ $tint }}; padding: 14px 32px; font-size: 16px; font-weight: 600; color: #ffffff; text-decoration: none">
                                        {{ __('mail.payment_failed.cta') }}
                                    </a>
                                </td>
                            </tr>
                        </table>
